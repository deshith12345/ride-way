...
import { ShieldAlert, ArrowLeft, Mail, Lock, Loader2 } from "lucide-react"
...
// ErrorContent calls useSearchParams() so it must be rendered client-side only.
// Wrapping with dynamic + ssr:false prevents the static generation pass from
// hitting the hook without a Suspense boundary.
const ErrorContent = dynamic(
    () =>
        Promise.resolve(function ErrorContentInner() {
    ...
    {
        ssr: false,
        loading: () => <div className="text-white"><Loader2 className="h-8 w-8 animate-spin" /></div>,
    }
)
